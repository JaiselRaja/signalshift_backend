...
async def list_turfs(
    ...
    """List turfs in the current tenant. Auth: any authenticated user."""
    return await svc.list_turfs(current_user.tenant_id, city)
...
async def get_turf(
    ...
    """Get turf details. Auth: any authenticated user."""
...
async def get_availability(
    ...
    """
    Real-time slot availability for a turf on a given date.
...
async def get_availability_range(
    ...
    """
    Availability for a date range (max 14 days).
    Auth: any authenticated user.
    """
